        ...
        batch.map(async (testimonial) => {
          try {
            ...
            if (!testimonial.Project) {
              console.log(`⏭️  Skipping testimonial ${testimonial.id} (no project)`);
              return;
            }

            // Skip if project doesn't have auto-moderation enabled
            if (!testimonial.Project.autoModeration) {
              console.log(`⏭️  Skipping testimonial ${testimonial.id} (auto-moderation disabled)`);
              return;
            }
            ...
            const result = await moderateTestimonial(
              ...
              testimonial.isOAuthVerified,
              {
                ...
                autoApproveVerified: testimonial.Project.autoApproveVerified || false,
                profanityFilterLevel: (testimonial.Project.profanityFilterLevel as 'STRICT' | 'MODERATE' | 'LENIENT') || 'MODERATE',
                moderationSettings: testimonial.Project.moderationSettings as any || {}
              }
            );
