...
const reconcileSubscriptionsLogger = logger.child({ module: 'script-reconcile-subscriptions' });
...
async function run() {
  try {
    ...
    reconcileSubscriptionsLogger.info({ result }, 'Subscription reconciliation result');
  } catch (error) {
    reconcileSubscriptionsLogger.error({ error }, 'Subscription reconciliation script failed');
    process.exitCode = 1;
  } finally {
