...
async function testWorkerStartup() {
  ...
  try {
    ...
    const { dailyDigestJob, startDailyDigestJob } = await import('../jobs/daily-digest.job.ts');
    const { reconciliationJob, startReconciliationJob } = await import('../jobs/reconciliation.job.ts');
    ...
    await import('../workers/outbox.worker.ts');
    await import('../workers/notification.worker.ts');
    await import('../workers/email.worker.ts');
