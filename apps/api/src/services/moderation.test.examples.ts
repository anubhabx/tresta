...
const negationTests = [
  ...
  {
    text: "This is not great at all",
    expectedSentiment: "negative",
    description: "Negated positive (not great)",
  },
  {
    text: "Not amazing, not wonderful, not good",
    expectedSentiment: "negative",
    description: "Multiple negated positives",
  },
  {
    text: "The product is hardly excellent",
    expectedSentiment: "negative",
    description: 'Negation with "hardly"',
  },

  // Negated negative → should be positive/neutral
  {
    text: "Not terrible, not bad at all",
    expectedSentiment: "neutral",
    description: "Negated negatives (not terrible)",
  },
  {
    text: "Never awful, never horrible",
    expectedSentiment: "neutral",
    description: "Negated severe negatives",
  },
  ...
  // No negation - standard positive
  {
    ...
    description: "Standard positive sentiment",
  },
  {
    text: "Excellent product, highly recommend",
    expectedSentiment: "very_positive",
  ...
  // No negation - standard negative
  {
    ...
    expectedSentiment: "very_negative",
    description: "Strong negative sentiment",
  },
  {
    text: "Bad quality, disappointing",
    expectedSentiment: "negative",
    description: "Standard negative sentiment",
  },
  ...
  // Mixed with negation context
  {
    ...
    expectedSentiment: "neutral",
    description: "Mixed negation (balanced)",
  },
  {
    text: "The service was barely acceptable",
    expectedSentiment: "negative",
    description: "Negation weakening positive",
  },
];
...
async function runProfanityTests() {
  ...
  console.log(
    `Success rate: ${((passed / profanityTests.length) * 100).toFixed(1)}%\n`,
  );
...
async function runSpamTests() {
  ...
  for (const testCase of spamTests) {
    ...
    const hasSpamFlags = result.flags.some(
      (f) =>
    ...
    if (testPassed) {
      ...
      console.log(
        `   Expected spam detection: ${testCase.shouldFlag}, Got: ${hasSpamFlags}`,
      );
    }
  ...
  console.log(
    `Success rate: ${((passed / spamTests.length) * 100).toFixed(1)}%\n`,
  );
...
async function runNegationTests() {
  ...
  for (const testCase of negationTests) {
    ...
    result.flags.forEach((flag) => {
      if (flag.toLowerCase().includes("very negative"))
        detectedSentiment = "very_negative";
      else if (flag.toLowerCase().includes("negative sentiment"))
        detectedSentiment = "negative";
      else if (
        flag.toLowerCase().includes("very_positive") ||
        flag.toLowerCase().includes("very positive")
      )
        detectedSentiment = "very_positive";
      else if (flag.toLowerCase().includes("positive sentiment"))
    ...
    if (testPassed) {
      ...
      console.log(`❌ FAIL: ${testCase.description}`);
      console.log(
        `   Expected: ${testCase.expectedSentiment}, Got: ${detectedSentiment}`,
      );
    }
  ...
  console.log(
    `Success rate: ${((passed / negationTests.length) * 100).toFixed(1)}%\n`,
  );
...
async function runAllTests() {
  ...
  const totalPassed =
    profanityResults.passed + spamResults.passed + negationResults.passed;
  const totalFailed =
    profanityResults.failed + spamResults.failed + negationResults.failed;
  const totalTests =
    profanityResults.total + spamResults.total + negationResults.total;
